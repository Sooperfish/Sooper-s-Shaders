...
/*
Soopers's shaders, derived from SonicEther v10 rc6
...
#define SHADOW_DARKNESS 0.51			//change shadow darkness , see .vsh for colors
#define SUNLIGHTAMOUNT 1.03				//change sunlight strength , see .vsh for colors
#define SHADOWDISTANCE 61.2 
#define SHADOW_CLAMP 0.5
#define SHADOW_RES 1024.0

/* SHADOWRES:1024 */				//affect shadow map resolution (change SHADOW_RES with it!)
/* SHADOWHPL:61.2 */				//affect shadow map draw distance (change SHADOWDISTANCE with it!)
  ...
  #define SSAO_STRENGTH 0.75               // Too much strength causes white highlights on extruding edges and behind objects
  #define SSAO_LOOP 0.75						// Integer affecting samples that are taken to calculate SSAO. Higher values mean more accurate shadowing but bigger performance impact
...
float bphong(vec3 ppos, vec3 lvector, vec3 normal) {
...
float tfade = abs(clamp(worldTime*1.0,3500.0,8500.0)-5500.0)/2000.0;
	shading = clamp(shading + (1.0-tfade)*0.5,0.0,1.0);			
*/	
}
...
wave = wave*(0.2+(1.0-abs(angle))*0.8);
...
refractedmask = 1.0;
	...
	color.rgb = mix(color.rgb, refracted.rgb, vec3(refractedmask));
						...
						spec =  bphong(fragposition.xyz,lightVector,normalize(normal+wave*0.05))* shading;
						  wspecmask = spec;
...
vec3 Sunlight_lightmap = sunlight_color*shading*SUNLIGHTAMOUNT + ambient_color*(1.0-shading)*(1.0-SHADOW_DARKNESS);
